...
def crop_video_start_label(frame, crop_start, crop_end, fvs, vid_name):
...
def create_and_save_json(crop_start, crop_end):

    try:
        with open("interactions.json", "r") as f:
            interactions = json.load(f)
        latest = interactions["latest"]
    ...
    cur_split = {}
    cur_split["start"] = crop_start
    cur_split["end"] = crop_end
    ...
    cur_split["groups"] = groups

    interactions["latest"] = latest
    interactions[latest] = cur_split

    with open("interactions.json", "w") as f:
        json.dump(interactions, f, indent=2, sort_keys=True)

    print("Saving interactions to interactions.json")